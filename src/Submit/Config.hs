...
instance Monad m => MonadMetrics (AppT m) where
    getMetrics = asks Submit.Config.configMetrics
...
connStr sfx = "host=localhost dbname=submit" <> sfx <> " user=submit password=submit port=5432"
